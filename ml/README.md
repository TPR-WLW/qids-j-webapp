...
## E. 探索ツールと「いまの正直な状態」（N=10, 陽性2）

特徴は十分揃った（ECG 相位/逐題、顔 blendshape・AU・幾何・視線・対称性・微表情）が、
**まだ学習・断定はできない**。理由は変わった：

- `ml/question_features.py` — 回答中の**逐題ダイナミクス**（設問ごとの HR 変化/心拍減速・表情/AU 反応）
  + 情緒負荷3設問（mood/self/suicide）の**合併窓 HRV**（pNN30/RMSSD/LF-HF, 基線比）。`ml/au_map.py` で blendshape→AU。
- `ml/anomaly_demo.py` — 顔特徴の screen（年齢を**偏相関で制御**）+ one-class の落とし穴の実演 + 可視化レポート。
- **置換検定の結論**: N=10・特徴38で max|偏相関|=0.82 だが、PHQ をシャッフルしても偶然で平均0.76・95%点0.90、
  多重比較込み **p≈0.29**。→ **探索の所見はノイズと区別できない**。特徴を増やすほど false-discovery が増える。

→ よって次は「特徴追加」ではなく **(1) 仮説を凍結（[`PREREGISTRATION.md`](./PREREGISTRATION.md)）→ (2) 陽性を含む新データ収集 → (3) 確認的検定（LOSO + 多重比較補正）」。
確認データ上での特徴選択は禁止。
